# Reasoning, Adaptation, or Imitation? A Three-Lens Analysis of Emergent Strategy in LLM Negotiation Agents

> **Status:** Work in progress
> **Last updated:** 2026-05-21

> **Final deliverable (LaTeX):** A condensed, submission-ready article version of this document lives at `docs/sn-article-template/negotiating_agents.tex` (compiles to **8 pages**, within the prof's 4–8 page limit; the PDF is git-ignored and regenerated on compile). It uses the required Springer Nature `sn-jnl` template, follows the 4-section structure, embeds 6 figures (as 3 side-by-side pairs), and is deliberately less repetitive and more focused on the **critical discussion of results** than this working document. This `presentation.md` remains the long-form working draft / source of record; the LaTeX file is the trimmed paper for submission.
...
This scenario is designed to elicit a broad range of communicative strategies typical of real-world negotiation contexts, making it a rich testbed for analysing agent behaviour. The bug serves as a natural information asymmetry pivot around which the negotiation dynamics unfold.
...
A third DeepSeek-R1 instance acts as an **Observer/Arbitrator**. After Phase 1, the Observer reads all negotiations and scores each agent's performance on a 1–5 scale. The scoring prompt supplies the full transcript, the scenario description, and both persona labels, but no explicit rubric: scoring criteria are implicit, with the model expected to infer what counts as good performance from persona and context (e.g. a Maximiser seller should maximise price), and required to provide a written rationale for each score. For each role, the 1–2 sessions with the highest score are selected and the Observer extracts: (a) an abstract natural-language description of the winning tactic, and (b) representative dialogue snippets.
...
*Level 1 — dialogue grounding (offline, regex + token matching).* For each turn N we extract a set of **anchor tokens** from the transcript visible up to turn N-1: prices (e.g. `7500`), percentages, and content-bearing words (≥ 4 characters, stop-words excluded). We then measure how many of those anchors appear inside the turn-N CoT. The resulting **grounding score** (anchors-found ÷ anchors-available) operationalises a sharp distinction the older flat indicator counts could not capture: a generic CoT such as *"I need to maximise price"* cites at most two anchor tokens against the full pool accumulated in the dialogue history, yielding a score near zero; one such as *"I need to adjust because they just named 6500, which undercuts my 7000 floor"* references several dialogue-specific anchors — `6500`, `named`, `7000`, `floor` — and scores substantially higher. A secondary metric, **conditional specificity**, focuses on conditional clauses (`if`, `otherwise`, `would ... if`, `in case`): for each turn it computes the fraction of those clauses that contain at least one anchor token — a turn that raises no conditional scoring zero — and reports the mean of this per-turn value across turns. It thus rewards reasoning that is both *contingent* and dialogue-anchored ("If they refuse 7500 again, I will ...") while penalising generic hedging ("If I push too hard, ..."); because conditional-free turns score zero, the metric also reflects how often the agent reasons conditionally, not only how anchored the conditionals it does raise are. A coarse **CoT length** metric tracks the raw character count per turn as a cognitive-effort proxy, on the rationale that turns requiring more situational reasoning tend to produce longer deliberation.
...
**Conditional specificity.** Across all four configurations, the mean per-turn conditional specificity is *higher* in Phase 1 than in Phase 2 (approximately 0.47 to 0.30 for configuration A, 0.38 to 0.27 for B, 0.37 to 0.08 for C, and 0.37 to 0.24 for D). This is counter-intuitive — one might expect that learning from successful peers would make reasoning more context-dependent — but it admits a coherent explanation. In the zero-shot baseline, lacking a prescribed tactic, agents reason contingently ("if the buyer accepts I concede, otherwise I hold firm"), and these conditionals naturally reference concrete dialogue anchors. In Phase 2 the tactic is already supplied in the prompt, so reasoning becomes directive ("execute the planned move") rather than conditional, and the conditionals that remain tend to be unanchored boilerplate ("if I push too hard"). 
